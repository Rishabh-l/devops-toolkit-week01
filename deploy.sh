...
# Dynamically calculate absolute project root path
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

echo "🚀 Starting Automated Deployment for Build 2..."
echo "📍 Project Root Directory resolved to: $SCRIPT_DIR"

# Dynamically template paths inside systemd files to avoid hardcoding errors
...
# Ensure sysreport timer configuration is copied and enabled
...
# Configure and deploy Nginx Reverse Proxy securely
